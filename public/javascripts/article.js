//將文章內容輸出到頁面上，供getArticle()呼叫
function initArticle(data) {
    //文章標題及內容
    var editArticle = '';
    var delArticle = '';
    if (data.account == $.cookie('userID')) {
        editArticle = `<a class="person" href="javascript:showArticlerow()">修改文章
                      </a>`;
        delArticle = `<a class="person" href="javascript:delArticle()" >刪除文章
                     </a>`;
    }
    ...
    data.comment.forEach(function (com) {
        ...
        if (com.account == $.cookie('userID')) {
            editbtn = 
            `<a class="person" href="javascript:showEditrow('${com.id}')">修改回應</a>`;

            delbtn = 
            `<a class="person" href="javascript:delComment('${com.id}')">刪除回應</a>`;
        }

        var commentdate = new Date(com.date);
        commentdate = (commentdate.getMonth() + 1) + '月' + commentdate.getDate() + '日' + commentdate.getHours() + ':' + commentdate.getMinutes();
        comment = `<div id="${com.id}">
                       <a class="like btn ${likeclass}" style="float:right">
                       &#128077;
                           <span id="like_count">${com.like.length}</span>
                       </a>
                       ...
                       <div>
                           <h3 style="margin-bottom:0;margin-top:0;">
                               ${com.name}
                               ( <a class="person" href="blog.html?account=${com.account}">${com.account}</a> )
                               ${editbtn + '    ' + delbtn}
                           </h3>${commentdate}
                       </div>
...
$('#a_like').on('click', function () {
    ...
    $.post("/blog/pushlike", { '_id': getUrlVal("_id"), 'account': $.cookie('userID') }, function (res) {
        if (res.status == 0) {
